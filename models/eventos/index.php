...
class Index_Model extends Model
{
	...
	private function crearTablaRegistros($clave)
	{
		$qry = "CREATE TABLE IF NOT EXISTS `smc_reg_%s` (
			...
			`clave_asociada` VARCHAR(125) NULL ,
			...
			`pais_RS_nombre` VARCHAR(175) NULL ,
	...
	private function crearTablaReservaciones($clave)
	{
		$qry = "CREATE TABLE IF NOT EXISTS `smc_reg_%s_reservaciones` (
			  ...
			  `res_numAdultos` TEXT NULL ,
			  `res_numMenores` TEXT NULL ,
			  ...
			  `res_formaPago` VARCHAR(10) NULL DEFAULT '' ,
	...
	private function crearTablaRFID($clave)
	{
		$qry = "CREATE TABLE IF NOT EXISTS `smc_reg_%s_RFID` (
			...
			`id_tag` varchar(24) COLLATE utf8_unicode_ci NOT NULL COMMENT 'ID para el RFID',
			`id_registro` int(11) DEFAULT NULL COMMENT 'ID del registro',
			`fecha_acceso` varchar(10) COLLATE utf8_unicode_ci NOT NULL,
			`hora_acceso` varchar(5) COLLATE utf8_unicode_ci NOT NULL,
			`fechaHora_acceso` datetime DEFAULT NULL,
			`numero_antena_acceso` tinyint(3) DEFAULT NULL,
			`fechaHora_salida` datetime DEFAULT NULL,
			`numero_antena_salida` tinyint(3) DEFAULT NULL,
			`numero_sesion` tinyint(2) DEFAULT NULL COMMENT 'Numero de sesion',
			`nombre_sesion` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
			`status` tinyint(1) DEFAULT '0' COMMENT '1 = Entrada, 2 = Salida',
			PRIMARY KEY (`id_RFID`),
			KEY `index_id_tag` (`id_tag`)
