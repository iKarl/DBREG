...
class UploadHandler
{
    ...
    protected function validate($uploaded_file, $file, $error, $index)
    {
        ...
        /**
         * Valida el mimetype del archivo
         * Addon personal
         */
        if (!empty($this->options['file_types']) && !in_array($file->type, $this->options['file_types']))
        {
            $file->error = 'fileTypes';
    ...
    protected function rename_file($file)
    {
        if (isset($this->options['new_name']) &&
            !empty($this->options['new_name']))
        {
            $ext = substr($file, strrpos($file, '.') + 1);
            $file = $this->options['new_name'] . "." . strtolower($ext);
        }

        return $file;
    }

    public function delete_whit_func()
    {
        $file_name = isset($_REQUEST['file']) ?
            basename(stripslashes($_REQUEST['file'])) : null;
